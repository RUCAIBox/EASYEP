...
if os.path.exists(current_fp):
    with open(current_fp,'r') as fp:
        experts_list = json.loads(fp.read())
else:
    experts_list=[[1.0 for _ in range(256)] for _ in range(58)]
...
class DeepseekV2MoE(nn.Module):
    ...
    def __init__(
        ...
        self.experts_info = nn.Parameter(torch.tensor(experts))
...
class DeepseekV2DecoderLayer(nn.Module):
    ...
    def __init__(
        ...
        layer_id: int,
        quant_config: Optional[QuantizationConfig] = None,
        experts_list: Optional[list] = None
    ) -> None:
        super().__init__()
        ...
        if (
            ...
            self.mlp = DeepseekV2MoE(config=config, quant_config=quant_config, experts = experts_list[layer_id - 3])
        else:
